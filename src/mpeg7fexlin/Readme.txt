MPEG-7 Low Level Feature Extraction - Command Line Tool (Linux executable)
...
e-mail: bastan@cs.bilkent.edu.tr or mubastan@gmail.com

web:www.cs.bilkent.edu.tr/~bastan
...
This command line tool for Linux is compiled (C++, gcc 4.3 on Ubuntu 9.04 distribution) 
using the MPEG-7 Low Level Feature Extraction library (C++, also planned to be released on the same web site when it is ready to be released), 
...
The library & executable uses OpenCV 1.0 (Open Computer Vision Library) for image/video handling, therefore the runtime libraries
are required to be in the LD_LIBRARY_PATH.
In bash or ksh, use export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:your_shared_lib_path, where your_shared_lib_path is the path to the runtime libraries.

OpenCV 1.0 was compiled with FFmpeg(0.4.9), therefore FFmpeg runtime libraries are also needed, and therefore included in the package ('solibs' directory).
Hence, you do not need to install OpenCV or FFmpeg, just use the provided libs..
Soon, the executable will be compiled with the latest versions of OpenCV (currently 2.0).

You may need to install the image handling libraries (libjpeg, libtiff, libpng, etc. -- used by OpenCV) if they are not already installed.
...
MPEG7Fex featureType featureParameters imageListFile outputFile

For instance, to extract Color Structure Descriptor from a set of images, run the following command:

MPEG7Fex CSD 64 imageList.txt CSD.txt
...
./images/Ordekler.jpg
/home/bastan/.../images/ari-cicek2.jpg
/home/bastan/.../images/agustos_bocegi.jpg

(The images can be specified by their full paths or relative paths, relative to the MPEG7Fex executable)
...
MPEG7Fex CSD 64 imageList.txt CSD.txt
or
MPEG7Fex CSD imageList.txt CSD.txt
...
MPEG7Fex SCD 128 imageList.txt SCD.txt
or 
MPEG7Fex SCD imageList.txt SCD.txt
...
MPEG7Fex CLD numberOfYCoeff numberOfCCoeff images.txt CLD.txt

MPEG7Fex CLD 64 28 imageList.txt CLD.txt
or
MPEG7Fex CLD imageList.txt CLD.txt
...
Example (see descriptors/CLD.txt, run as MPEG7Fex CLD 6 3 imageList.txt descriptors/CLD.txt)
...
MPEG7Fex DCD normalizationFlag varianceFlag spatialFlag numBin1 numBin2 numBin3 images.txt DCD.txt

MPEG7Fex DCD 1 0 1 32 32 32 imageList.txt DCD.txt
or
MPEG7Fex DCD 1 0 1 imageList.txt DCD.txt
or
MPEG7Fex DCD imageList.txt DCD.txt
...
MPEG7Fex HTD layerFlag imageList.txt HTD.txt

MPEG7Fex HTD 1 imageList.txt HTD.txt
or
MPEG7Fex HTD imageList.txt HTD.txt
...
Muhammet Bastan, Hayati Cam, Ugur Gudukbay, Ozgur Ulusoy, 
“An MPEG-7 Compatible Video Retrieval System with Integrated Support for Complex Multimodal Queries,”
IEEE MultiMedia, to appear, 2009.
